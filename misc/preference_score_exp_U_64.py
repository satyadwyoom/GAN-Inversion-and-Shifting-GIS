...
batch_size = 1
numofworkers= 4
gan_weights= '../PROGAN_AM_Fashion/Model_log_base/models/depth_6_epoch_50.bin' 
...
data_loader_train = DataLoader(train_dataset, batch_size = 1000, shuffle=True, num_workers = numofworkers, drop_last=True)
...
### for pref images
# unpref_images, _, u, _, _ = next(iter(data_loader_train))
...
for u_i in range(len(u)):
    ...
    rs_path = '../AIP/models/ckpt/Newamazon_K_64_100_19.tar'
    ...
    user_nu = u[u_i]
    real_img = unpref_images[u_i].unsqueeze(dim=0).cuda()
    ...
    Preference_score1 = torch.mul(thetau[user_nu],rs_feat1).sum(1)

    ### User Preference
    rs_in2 = real_img
    rs_feat2 = rs_model(rs_in2)
    # Preference_score = torch.log(torch.sigmoid(torch.mul(thetau[user_nu],rs_feat).sum(1))).mean()
    Preference_score2 = torch.mul(thetau[user_nu],rs_feat2).sum(1)
    ...
    for step in range(0, n_steps, 1): 
        ...
        # Preference_score = torch.log(torch.sigmoid(torch.mul(thetau[user_nu],rs_feat).sum(1))).mean()
        Preference_score = torch.mul(thetau[user_nu],rs_feat).sum(1)
        ...
        if step%100==0:
            print("user: {} iter{}: loss: {:.2f}| logit_loss: {:.2f}| shift_loss: {:.2f}| Preference_score: {:.2f}".format(user_nu, step,loss.item(),logit_loss.item(),shift_loss.item(),Preference_score.mean().item()))
    ...
    item_base_preference = Preference_score2.cpu().detach().unsqueeze(dim=0)
    item_user_pref.append(item_base_preference)

    base_gen_images.append(dlatent_img.detach().cpu())
    item_images.append(real_img.detach().cpu())
...
item_user_pref = torch.cat(item_user_pref, dim=0)
base_gen_images = torch.cat(base_gen_images, dim=0)
item_images = torch.cat(item_images, dim=0)
shifted_images = torch.cat(shifted_images, dim=0)

# torch.save(all_user_pref, 'raw_data_exp/preference_score.pt')
# torch.save(base_user_pref, 'raw_data_exp/base_preference_score.pt')
# torch.save(base_gen_images, 'raw_data_exp/base_images.pt')
# torch.save(shifted_images, 'raw_data_exp/shifted_images.pt')

torch.save(all_user_pref, 'raw_data_exp/preference_score_U_up_64.pt')
torch.save(base_user_pref, 'raw_data_exp/base_preference_score_U_up_64.pt')
torch.save(item_user_pref, 'raw_data_exp/item_preference_score_U_up_64.pt')
torch.save(base_gen_images, 'raw_data_exp/base_images_U_up_64.pt')
torch.save(item_images, 'raw_data_exp/item_images_U_up_64.pt')
torch.save(shifted_images, 'raw_data_exp/shifted_images_U_up_64.pt')

# torch.save(all_user_pref, 'raw_data_exp/preference_score_U_p_64.pt')
# torch.save(base_user_pref, 'raw_data_exp/base_preference_score_U_p_64.pt')
# torch.save(item_user_pref, 'raw_data_exp/item_preference_score_U_p_64.pt')
# torch.save(base_gen_images, 'raw_data_exp/base_images_U_p_64.pt')
# torch.save(item_images, 'raw_data_exp/item_images_U_p_64.pt')
# torch.save(shifted_images, 'raw_data_exp/shifted_images_U_p_64.pt')

print(all_user_pref.shape)
print(base_user_pref.shape)
print(item_user_pref.shape)
